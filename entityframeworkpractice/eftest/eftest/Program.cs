...
namespace eftest
{
    class Program
    {
        ...
        {
            Database.SetInitializer(new MigrateDatabaseToLatestVersion<eftestContext, eftest.Migrations.Configuration>());
        ...
        {
            //1.实例化实体上下文
            eftestContext dbContext = new eftestContext();
            ...
            //更新
            dbContext.Users.Add(u);//附加到上下文进行跟踪管理
        ...
        public static void select()
        {
            ...
            {
                Console.WriteLine(stu.ID + " " + stu.Name + " " + stu.Course + " " + stu.Score + " "+stu.Age+" ");
            }
            //user
            var queryuser = from user in dbContext.Users
                            where user.DisplayName.Length > 1
                            select user;
            foreach (var u in queryuser.ToList())
            {
                Console.WriteLine(u.DisplayName + "    " + u.Username+"    ");
            }
